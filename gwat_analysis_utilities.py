...
def CoM_error(profile,CoM):
	top,bottom = sum(profile*arange(len(profile))),sum(profile)
	err_top = sqrt(sum(arange(len(profile))**2*profile))
	err_bottom = sqrt(sum(profile))
	sigma_CoM = CoM*sqrt((err_top/top)**2 + (err_bottom/bottom)**2)
	return sigma_CoM

def get_CoM_errors(img):
	profile_x,profile_y = sum(img,axis = 0),sum(img,axis = 1)
	xCoM,yCoM = CoM(profile_x),CoM(profile_y)
	xCoM_err,yCoM_err = CoM_error(profile_x,xCoM),CoM_error(profile_y,yCoM)	
	return xCoM_err,yCoM_err
...
def xray_plot(img):
	plt.imshow(img,cmap = plt.get_cmap('nipy_spectral'),norm = LogNorm())
